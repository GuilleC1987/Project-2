...
def load_data(messages_filepath, categories_filepath):
    ...
    # separating categories column into 36 different columns
    ...
    category_colnames = row.apply(lambda x: x[slice(len(x)-2)])
    ...
    for column in categories:
        categories[column] = categories[column].apply(lambda x: pd.to_numeric(x[slice(len(x)-1,len(x))]))
...
def clean_data(df):
    ...
    df = df[df['related']!=2]
    #dropping child_alone column because only have cero values
